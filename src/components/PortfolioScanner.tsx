...
import { easeOut, fadeIn, fadeUp, staggerContainer } from "@/lib/motion";
import { labelForScope } from "@/lib/multichain/scopes";
import { AnimatedUsd } from "./AnimatedUsd";
import { BotTipGate } from "./BotTipGate";
import { TokenIcon } from "./TokenIcon";
import { WalletHub } from "./WalletHub";
import {
  isWalletTipUnlocked,
} from "@/lib/botchain/unlock";
import { BOT_TIP_AMOUNT } from "@/lib/botchain/config";
...
export function PortfolioScanner() {
  ...
  const walletLabel = walletKind === "phantom" ? "Phantom" : "MetaMask";
  ...
  useEffect(() => {
    ...
    setTipUnlocked(isWalletTipUnlocked(tipPayer));
  ...
  const rebuildMulti = useCallback(
    (
      ...
      for (const t of extraTokens) {
        ...
        existing.valueUsd =
          existing.valueUsd == null && t.valueUsd == null
            ? null
            : (existing.valueUsd ?? 0) + (t.valueUsd ?? 0);
      ...
      const extraUsd = extraTokens.reduce((s, t) => s + (t.valueUsd ?? 0), 0);
    ...
    try {
      ...
      for (let i = 0; i < wallets.length; i += BATCH_SIZE) {
        ...
        rebuildMulti(nextSlices, extras(), [
          `Scanned ${done}/${totalJobs} wallets (EVM quick pass).`,
          ...(errCount
      ...
      // Phase 2: deepen top EVM wallets + any that only got native fallback / errors.
      ...
      const uniqueEnrich = [...new Set(enrichTargets)].slice(0, ENRICH_TOP + 10);
        ...
        for (let i = 0; i < uniqueEnrich.length; i += BATCH_SIZE) {
          ...
          if (res.ok) {
            for (const row of data.results) {
              ...
              // Keep richer of the two passes (indexed can briefly fail under 429).
              if (
                row.ok &&
          ...
          rebuildMulti(nextSlices, extras(), [
            `Deepened ${enrichDone}/${uniqueEnrich.length} high-value wallets (fuller Ankr pass).`,
          ]);
      ...
      if (epoch !== scanEpoch.current) return;
      const fallbackN = nextSlices.filter((s) =>
        (s.provider || "").includes("native-rpc"),
      ).length;
      const errN = nextSlices.filter((s) => s.status === "error").length;
      rebuildMulti(nextSlices, extras(), [
        `Finished ${wallets.length} wallet(s). ${walletLabel} totals can still be higher if some networks weren’t approved on Connect.`,
        ...(fallbackN
          ? [`${fallbackN} wallet(s) used native-only fallback (ERC-20s may be missing).`]
          : []),
        ...(errN ? [`${errN} wallet(s) still failed the indexer.`] : []),
        ...(solanaAddrs.length === 0
          ? [`Solana not in ${walletLabel} session — reconnect and approve Solana.`]
          : []),
        ...(bitcoinAddrs.length === 0
          ? [`Bitcoin not in ${walletLabel} session yet — approve Bitcoin on Connect if offered.`]
          : []),
        ...(suiAddrs.length === 0 && walletKind === "phantom"
          ? ["Sui not in Phantom session yet — approve Sui when Phantom prompts."]
          : []),
      ]);
      setLoading(false);
      setScanProgress(null);
  ...
  const displayTokens = useMemo(() => {
    ...
    if (viewMode === "wallet" && activeWallet !== "all") {
      const slice = slices.find((s) => s.address === activeWallet);
      return (slice?.tokens ?? []) as Array<
        TokenBalance & { walletCount?: number }
      >;
    }
    return multi.combinedTokens;
  ...
  const displayTotals = useMemo(() => {
    ...
    if (viewMode === "wallet" && activeWallet !== "all") {
      const slice = slices.find((s) => s.address === activeWallet);
      return {
        value: slice?.totalValueUsd ?? 0,
        tokens: slice?.tokenCount ?? 0,
        chains: slice?.chainCount ?? 0,
      };
    }
    return {
      value: multi.totalValueUsd,
      tokens: multi.tokenCount,
      chains: multi.chainCount,
    };
  }, [multi, viewMode, activeWallet, slices]);
  ...
  return (
    ...
    >
      <div className="scan-top">
        <div>
          ...
          <div className="eco-row">
            ...
            {scanSolana.length ? (
              <span className="eco-chip">Solana</span>
            ) : null}
            ...
            {walletKind ? (
              <span className="eco-chip">{walletLabel}</span>
            ) : null}
            {viewMode === "combined" ? (
            ...
            {evmOnly && scanSolana.length === 0 ? (
              <span className="eco-chip eco-chip-warn">EVM only</span>
            ) : null}
          </div>
        </div>
      ...
      <WalletHub
        ...
        totalValueUsd={multi?.totalValueUsd ?? 0}
      ...
      accounts.all.length > 0 ? (
        <details className="extra-chain-panel">
          <summary className="extra-chain-title">
            {walletLabel} network addresses ({accounts.all.length})
          </summary>
          <p className="extra-chain-copy">
            Each network can use a different address format — Solana / Bitcoin /
            Sui are never the same as your Ethereum 0x. Phantom EVM nets
            (Ethereum, Base, Polygon, Monad, Robinhood, HyperEVM) share one 0x.
          </p>
          <ul className="linked-addr-list">
            {Object.entries(accounts.byScope).map(([scope, addrs]) =>
              addrs.map((addr) => (
                <li key={`${scope}:${addr}`}>
                  <span className="eco-chip">{labelForScope(scope)}</span>
                  <code className="addr-full" title={addr}>
                    {addr}
                  </code>
                </li>
              )),
      ...
      <div className="extra-chain-panel">
        ...
        <p className="extra-chain-copy">
          Addresses come from {walletLabel} automatically — Solana, EVM,
          Bitcoin, Sui, and Robinhood (no paste). Approve every network Phantom
          or MetaMask offers when you connect.
        </p>
        ...
        {enabledEvm.length > 0 ? (
          <ul className="linked-addr-list">
            {enabledEvm.map((addr) => (
              <li key={`evm:${addr}`}>
                <span className="eco-chip">EVM</span>
                <code className="addr-full" title={addr}>
                  {addr}
                </code>
              </li>
        ...
        {enabledSolana.length > 0 || scanSolana.length > 0 ? (
          <ul className="linked-addr-list">
            {[
              ...new Set([...enabledSolana, ...scanSolana]),
            ].map((addr) => (
              <li key={`sol:${addr}`}>
                <span className="eco-chip">Solana</span>
                <code className="addr-full" title={addr}>
        ...
        {scanTron.length > 0 ? (
          <ul className="linked-addr-list">
            {scanTron.map((addr) => (
              <li key={`tron:${addr}`}>
                <span className="eco-chip">Tron</span>
                <code className="addr-full" title={addr}>
                  {addr}
                </code>
              </li>
        ...
        {scanSui.length > 0 ? (
          <ul className="linked-addr-list">
            {scanSui.map((addr) => (
              <li key={`sui:${addr}`}>
                <span className="eco-chip">Sui</span>
                <code className="addr-full" title={addr}>
                  {addr}
                </code>
              </li>
        ...
        {(scanSolana.length === 0 ||
          ...
          <div className="extra-chain-form" style={{ marginTop: "0.75rem" }}>
            ...
            >
              {isPending ? "Discovering…" : `Refresh from ${walletLabel}`}
            </button>
          </div>
        )}
      </div>
              ...
              <div className="value-block">
                ...
                {!tipUnlocked ? (
                  <p className="value-hint">
                    Total is visible — tip {BOT_TIP_AMOUNT} BOT to see tokens
                    and chains.
                  </p>
                ) : null}
              </div>
              ...
              {!tipUnlocked ? (
                <div className="portfolio-locked-panel">
                  ...
                  <p>
                    ...
                    {BOT_TIP_AMOUNT} BOT on BOT Chain to reveal which tokens you
                    hold and on which networks.
                  </p>
                </div>
              ) : (
                <>
                ...
                return (
                  <div className="coverage-banner" role="status">
                    <p className="coverage-title">
                      Why this can be below {walletLabel}
                    </p>
                    <ul>
                      {noSol || noBtc || noSui ? (
                        <li>
                          <strong>Non-EVM still waiting</strong> — disconnect and
                          Connect {walletLabel} again; approve Solana / Bitcoin
                          / Sui when prompted (no paste).
                        </li>
                      ) : null}
                      {fallbackN > 0 ? (
                        <li>
                          <strong>{fallbackN} wallet(s)</strong> only got native
                          balances (Ankr rate-limit) — ERC-20s like USDC may be
                          missing. Rescan later or Deep scan those wallets.
                        </li>
                      ) : null}
                      {errN > 0 ? (
                        <li>
                      ...
                      <li>
                        EVM L2s usually share one 0x — we enrich HyperEVM,
                        Monad, Sei, Fraxtal, Flare, Rootstock, and more on the
                        deep pass. Non-EVM nets need their own linked addresses.
                      </li>
                    </ul>
                    {multi.warnings.length > 0 ? (

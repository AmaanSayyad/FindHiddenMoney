...
export function portfolioToSlice(
  ...
  const empty = result.tokenCount === 0;
  return {
    address,
    label,
...
export function buildMultiPortfolio(
  ...
  return {
    ...
    insights: {
      ...
      emptyCount: wallets.filter((w) => w.status === "empty").length,
      errorCount: wallets.filter((w) => w.status === "error").length,

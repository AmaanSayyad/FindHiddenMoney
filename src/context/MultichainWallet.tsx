...
export type WalletKind = "metamask" | "phantom" | null;
...
function syncRosterFromAccounts(
  prev: RosterWallet[],
  accounts: MultichainAccounts,
  source: "metamask" | "phantom" = "metamask",
): RosterWallet[] {
...
export function MultichainWalletProvider({ children }: { children: ReactNode }) {
  ...
  const addSolanaAddresses = useCallback(
    (addrs: string[], source: "phantom" | "metamask" | "manual" = "phantom") => {
  ...
  const applyConnectedAccounts = useCallback(
    (next: MultichainAccounts, kind: "metamask" | "phantom") => {
      setAccounts(next);
      setWalletKind(kind);
  ...
  const connect = useCallback(async () => {
    ...
    setError(null);
    try {
      const next = await createMultichainSession();
      if (next.all.length === 0) {
        throw new Error("No accounts returned from MetaMask");
      }
      applyConnectedAccounts(next, "metamask");
      // Second pass — Solana/BTC/Tron sometimes settle after the first prompt.
      void refreshDiscoveredAccounts(next).then((enriched) => {
        applyConnectedAccounts(enriched, "metamask");
      });
    } catch (err) {
